...
const New_fifth = function ({ navigation }) {
  ...
  const [ssize, ssetsize] = useState(data.size);

  const cart = useSelector((state) => state.token.change);

  useEffect(() => {
    console.log(data, '아이디좀')
  ...
  useEffect(() => {
    axios.get(`http://192.168.45.191:3000/like_total_lost/${data._id}`, {
      headers: {
  ...
  const go_heart = function () {
    ...
    axios.post('http://192.168.45.191:3000/like', {
      id: data._id
    }, {
      headers: {
        'Authorization': `Bearer ${token}`
